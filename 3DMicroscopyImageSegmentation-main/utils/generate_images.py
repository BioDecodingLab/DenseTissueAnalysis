...
def get_smooth_variations(shape, binning, scale=5):
    ...
    smoothed_variation = zoom(smoothed_variation_temp, zoom_factors, order=1)  # order=1 for bilinear interpolation
...
def simulate_uneven_staining(binary_mask, local_variation_scale=5, z_decay_rate=0.999):
    ...
    local_variation = simulate_local_variations(binary_mask.shape, binnings=[1, 2, 4, 8], scale=local_variation_scale)
...
def add_BG(stained_image, background_level, image_intensity_scale, local_variation_scale=5):
    BG = np.ones(stained_image.shape) *  background_level  * image_intensity_scale # Contsnat background
    BG_noisy = BG * simulate_local_variations(stained_image.shape, binnings=[1,2], scale=local_variation_scale) # add local noise
...
def generate_SNR_images(noisy_poisson_image, convolved_image, binary_mask, snr_targets, snr_tolerance, std_dev, max_iterations, basename, out_dir_masks, out_dir_images):
  ...
  for snr_target in snr_targets:
    ...
    print(f" Final SNR: {snr:.2f} (Target SNR: {snr_target}), Gaussian std_dev: {std_dev:.4f}, Iterations: {iteration}")# Define Functions
...
# Convolution
def convolve_with_psf(image_3D, psf, conv_type='3D'):
    """
    Convolve the 3D image stack with 2D or 3D PSF depending on conv_type.
    - conv_type: '3D' for full 3D convolution
                 '2D' for per-plane convolution using center Z-plane of the PSF
    """
    psf = normalize_psf(psf)

    if conv_type == '3D':
        # Padding size for each dimension
        pad_size = [p // 2 for p in psf.shape]
        padded_image = np.pad(image_3D, [(p, p) for p in pad_size], mode='reflect')
        convolved_padded = fftconvolve(padded_image, psf, mode='same')
        slices = tuple(slice(p, -p) for p in pad_size)
        convolved_image = convolved_padded[slices]
        ...
        # Extract the center Z-plane of the 3D PSF
        center_z = psf.shape[0] // 2
        psf_2D = psf[center_z, :, :]
        pad_y, pad_x = psf_2D.shape[0] // 2, psf_2D.shape[1] // 2
        convolved_image = np.zeros_like(image_3D)

        for z in range(image_3D.shape[0]):
            plane = image_3D[z, :, :]
            padded_plane = np.pad(plane, [(pad_y, pad_y), (pad_x, pad_x)], mode='reflect')
            convolved_plane = fftconvolve(padded_plane, psf_2D, mode='same')
            convolved_image[z] = convolved_plane[pad_y:-pad_y, pad_x:-pad_x]

    else:
        raise ValueError("conv_type must be '3D' or '2D'")
...
def generate_SNR_images(noisy_poisson_image, convolved_image, binary_mask, snr_targets, snr_tolerance, std_dev, max_iterations, basename, out_dir_masks, out_dir_images):
  ...
  for snr_target in snr_targets:
    ...
    print(f" Final SNR: {snr:.2f} (Target SNR: {snr_target}), Gaussian std_dev: {std_dev:.4f}, Iterations: {iteration}")        
...
def simulate_images(
    ...
    for img_path, mask_path in zip(image_files, masks_files):
        ...
        stained_image = simulate_uneven_staining(image, local_variation_scale=local_variation_scale, z_decay_rate=z_decay_rate)
        #save_image(stained_image, "staining.tif", out_dir_images)
    
        # Step 2 : Add BG
        stained_image = add_BG(stained_image, background_level, image_intensity_scale, local_variation_scale=5)
        ...
        convolved_image = convolve_with_psf(stained_image, psf, conv_type)
        ...
        generate_SNR_images(noisy_poisson_image, convolved_image, binary_mask, snr_targets, snr_tolerance, std_dev, max_iterations, file_name, out_dir_masks, out_dir_images)
