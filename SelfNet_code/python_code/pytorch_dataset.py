...
def create_train_data(train_data_path,batch_size,min_v,max_v, pairedDataSets):

    dataset=np.load(train_data_path)
    train_data=torch.utils.data.DataLoader(ImageDataset_numpy(dataset,pairedDataSets,min_v,max_v),batch_size=batch_size,shuffle=True)
